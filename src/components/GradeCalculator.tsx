...
export default function GradeCalculator() {
  ...
  // Detailed mode states (Prelim, Midterm, Final breakdowns)
  ...
  const calculateDetailedTermGrade = (period: 'prelim' | 'midterm' | 'final') => {
    ...
    if (classType === 'lecture') {
      ...
      // Total Term Grade = 60% Lecture + 40% Lab
      const totalGrade = (lectureGrade * 0.6) + (labGrade * 0.4);
  ...
  if (mode === 'quick') {
    ...
    // Formula: 30% Prelim + 30% Midterm + 40% Final
    finalPercentage = (prelimPct * 0.3) + (midtermPct * 0.3) + (finalPct * 0.4);
  } else {
    ...
    finalPercentage = (computedPrelim * 0.3) + (computedMidterm * 0.3) + (computedFinal * 0.4);
  }
      ...
      <div className="bg-gradient-to-r from-navy-700 to-navy-800 px-6 py-5 text-white flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-navy-100">Calculate final trimestral marks using ACLC formulas</p>
          </div>
        </div>
        <button
          onClick={handleReset}
          ...
          <div className="lg:col-span-8 space-y-6">
            ...
            {mode === 'quick' ? (
              ...
              <div className="space-y-6">
                ...
                {/* Grid for Prelim, Midterm, Final blocks */}
                <div className="space-y-6">
                  {/* Period Block Generator */}
                  {(['prelim', 'midterm', 'final'] as const).map((period) => (
                    <div key={period} className="bg-white rounded-xl border border-slate-100 shadow-sm overflow-hidden">
                      <div className="bg-slate-100/80 px-4 py-2.5 border-b border-slate-100 flex justify-between items-center">
                        <span className="font-display font-semibold text-xs uppercase tracking-wider text-slate-700">
                          {period} term period {period === 'final' ? '(40% Weight)' : '(30% Weight)'}
                        </span>
                      ...
                      <div className="p-4 grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div className="space-y-3">
                          <h4 className="text-[11px] font-bold uppercase tracking-wider text-navy-700 flex items-center gap-1">
                            ...
                            Lecture Scores (60% weight if Lab)
                          </h4>
              ...
              <div className="mt-4 border-t border-slate-200/60 pt-4">
                ...
                <div className="grid grid-cols-3 gap-1.5 text-[10px] text-center font-mono">
                  <div className="bg-white p-1.5 rounded border border-slate-200/50">
                    <span className="text-slate-400 block mb-0.5">Prelims</span>
                    <span className="text-slate-700 font-bold">{displayPrelim}%</span>
                  </div>
                  <div className="bg-white p-1.5 rounded border border-slate-200/50">
                    <span className="text-slate-400 block mb-0.5">Midterms</span>
                    <span className="text-slate-700 font-bold">{displayMidterm}%</span>
                  </div>
                  <div className="bg-white p-1.5 rounded border border-slate-200/50">
                    <span className="text-slate-400 block mb-0.5">Finals</span>
                    <span className="text-slate-700 font-bold">{displayFinal}%</span>
                  </div>
                </div>
              </div>
            </div>
